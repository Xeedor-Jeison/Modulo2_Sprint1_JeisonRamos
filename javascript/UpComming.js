...
fetch(misVariables.url)
    ...
    .then(data => {
        ...
        misFunciones.crearTarjetas(eventsFuture, misVariables.carrusel)

        let arrayCategory = Array.from(new Set(eventsFuture.map(event => event.category)))
        ...
        misVariables.divCheckbox.addEventListener("change", e => {
            ...
            if (checked.length === 0) {
                newArray = eventsFuture
            } else {
                newArray = misFunciones.filterCheckbox(eventsFuture, checked)
        ...
        misVariables.search.addEventListener("keyup", e => {
            ...
            let newArray = misFunciones.filterCheckbox(eventsFuture, checked)

            let nuevoArregloBuscar = misFunciones.FitlerText(newArray, e.target.value)

            let buscador = misFunciones.FitlerText(eventsFuture, e.target.value)
