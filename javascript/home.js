...
fetch(url)
    ...
    .then(data => {
        ...
        crearTarjetas(data.events, carrusel)
        ...
        divCheckbox.addEventListener("change", e => {
            ...
            if (checked.length === 0) {
                ...
                newArray = filterCheckbox(data.events, checked)
        ...
        search.addEventListener("keyup", e => {
            ...
            let newArray = filterCheckbox(data.events, checked)

            let nuevoArregloBuscar = FitlerText(newArray, e.target.value)

            let buscador = FitlerText(data.events, e.target.value)
