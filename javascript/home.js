...
fetch(url)
    ...
    .then(data => {
        ...
        misFunciones.crearTarjetas(data.events, misVariables.carrusel)
        ...
        misVariables.divCheckbox.addEventListener("change", e => {
            ...
            if (checked.length === 0) {
                newArray = data.events
            } else {
                newArray = misFunciones.filterCheckbox(data.events, checked)
        ...
        misVariables.search.addEventListener("keyup", e => {
            ...
            let newArray = misFunciones.filterCheckbox(data.events, checked)
